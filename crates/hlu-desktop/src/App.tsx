...
export function App() {
  ...
  // Load the persisted inventory on first paint.
  useEffect(() => {
    api
      .listDevices()
      .then(setDevices)
    ...
    <div className="app">
      <header className="topbar">
        <div className="brand">
          ...
          <div>
            <h1>Homelab Utils</h1>
            <p className="tagline">Discover · name · connect</p>
          </div>
        </div>
  ...
  return (
    <article className="device">
      ...
      <div className="device-ssh">
        <span className={`badge ssh-${device.ssh.status}`} title={device.ssh.banner ?? ""}>
